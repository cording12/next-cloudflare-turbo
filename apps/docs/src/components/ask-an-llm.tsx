  ...
  const items = useMemo(() => {
    const fullMarkdownUrl =
      typeof window !== "undefined"
        ...
        : "loading"
    let urlStr =
      typeof fullMarkdownUrl === "string"
        ? fullMarkdownUrl
        : fullMarkdownUrl.toString()
    if (urlStr.endsWith(".mdx")) {
      urlStr = urlStr.slice(0, -4)
    }
    const q = `Please read ${urlStr}, I want to ask questions about it.`
  ...
  return (
    <Popover>
      <PopoverTrigger
        ...
        onClick={() => {
          captureButtonClick("popover-click", { button_name: "Open in" })
        }}
      >
        Ask an LLM
